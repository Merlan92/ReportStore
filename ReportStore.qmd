...
if 'широта' in df.columns and 'долгота' in df.columns:
    df.rename(columns={'широта': 'lat', 'долгота': 'long'}, inplace=True)

def highlight_inactive(row):
    return ['background-color: blue'] * len(row) if row['status'] == 'Неактивен' else [''] * len(row)

df['type'] = df['id'].apply(lambda x: 'OR' if sum(c.isalpha() for c in str(x)) >= 2 else 'IR') 
df['status'] = df['status'].map({True: 'Активен', False: 'Неактивен'})
styled_output = df.style.apply(highlight_inactive, axis=1).set_table_styles([
    {'selector': 'thead th', 'props': [('color', 'white'), ('background-color', 'black')]}
])
df = df[['organisation', 'status', 'type', 'id', 'region', 'city', 'lat', 'long']].reset_index(drop=True)
...
dict(value=int(df['status'].eq('Неактивен').sum()))
```
...
plt.legend(['Активные', 'Не Активные'], title='Статус', fontsize=12)
plt.tight_layout();
plt.show();
```
